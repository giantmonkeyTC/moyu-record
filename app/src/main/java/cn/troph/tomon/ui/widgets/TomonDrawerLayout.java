...
public class TomonDrawerLayout extends ViewGroup {
    private static final String TAG = "TomonDrawerLayout";
...
//    private static final int FLAG_CLOSE_OPEN_DRAWER_ON_BACK_PRESSED_ENABLED = 1 << 25;
    ...
    private static final int SCROLL_RATIO_CONTENT_TO_DRAWER = 4 / 1;
    ...
    public TomonDrawerLayout(Context context) {
        this(context, null);
    }

    public TomonDrawerLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TomonDrawerLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs,
                R.styleable.TomonDrawerLayout, defStyleAttr, 0);
        if (ta.hasValue(R.styleable.TomonDrawerLayout_widthPercent_leftDrawer)) {
            mLeftDrawerWidthPercent = ta.getFloat(R.styleable
                    .TomonDrawerLayout_widthPercent_leftDrawer, UNSPECIFIED_DRAWER_WIDTH_PERCENT);
            checkDrawerWidthPercent(mLeftDrawerWidthPercent);
        }
        if (ta.hasValue(R.styleable.TomonDrawerLayout_widthPercent_rightDrawer)) {
            mRightDrawerWidthPercent = ta.getFloat(R.styleable.
                    TomonDrawerLayout_widthPercent_rightDrawer, UNSPECIFIED_DRAWER_WIDTH_PERCENT);
            checkDrawerWidthPercent(mRightDrawerWidthPercent);
        }
        if (ta.hasValue(R.styleable.TomonDrawerLayout_widthPercent_startDrawer)) {
            mStartDrawerWidthPercent = ta.getFloat(R.styleable
                    .TomonDrawerLayout_widthPercent_startDrawer, UNSPECIFIED_DRAWER_WIDTH_PERCENT);
            checkDrawerWidthPercent(mStartDrawerWidthPercent);
        }
        if (ta.hasValue(R.styleable.TomonDrawerLayout_widthPercent_endDrawer)) {
            mEndDrawerWidthPercent = ta.getFloat(R.styleable
                    .TomonDrawerLayout_widthPercent_endDrawer, UNSPECIFIED_DRAWER_WIDTH_PERCENT);
            checkDrawerWidthPercent(mEndDrawerWidthPercent);
        }
        if (ta.hasValue(R.styleable.TomonDrawerLayout_enabledInTouch_leftDrawer)) {
            mFlags |= FLAG_LEFT_DRAWER_TOUCH_ABILITY_DEFINED;
            if (ta.getBoolean(R.styleable.TomonDrawerLayout_enabledInTouch_leftDrawer, true)) {
                mFlags |= FLAG_LEFT_DRAWER_ENABLED_IN_TOUCH;
            }
        }
        if (ta.hasValue(R.styleable.TomonDrawerLayout_enabledInTouch_rightDrawer)) {
            mFlags |= FLAG_RIGHT_DRAWER_TOUCH_ABILITY_DEFINED;
            if (ta.getBoolean(R.styleable.TomonDrawerLayout_enabledInTouch_rightDrawer, true)) {
                mFlags |= FLAG_RIGHT_DRAWER_ENABLED_IN_TOUCH;
            }
        }
        if (ta.hasValue(R.styleable.TomonDrawerLayout_enabledInTouch_startDrawer)) {
            mFlags |= FLAG_START_DRAWER_TOUCH_ABILITY_DEFINED;
            if (ta.getBoolean(R.styleable.TomonDrawerLayout_enabledInTouch_startDrawer, true)) {
                mFlags |= FLAG_START_DRAWER_ENABLED_IN_TOUCH;
            }
        }
        if (ta.hasValue(R.styleable.TomonDrawerLayout_enabledInTouch_endDrawer)) {
            mFlags |= FLAG_END_DRAWER_TOUCH_ABILITY_DEFINED;
            if (ta.getBoolean(R.styleable.TomonDrawerLayout_enabledInTouch_endDrawer, true)) {
                mFlags |= FLAG_END_DRAWER_ENABLED_IN_TOUCH;
            }
        }
        setContentSensitiveEdgeSize(ta.getDimensionPixelSize(R.styleable
                .TomonDrawerLayout_contentSensitiveEdgeSize, (int) (DEFAULT_EDGE_SIZE * mDp + 0.5f)));
        setContentFadeColor(ta.getColor(R.styleable
                .TomonDrawerLayout_contentFadeColor, DEFAULT_FADE_COLOR));
        setDuration(ta.getInteger(R.styleable.TomonDrawerLayout_duration, DEFAULT_DURATION));
//        setCloseOpenDrawerOnBackPressedEnabled(ta.getBoolean(R.styleable
//                .TomonDrawerLayout_closeOpenDrawerOnBackPressedEnabled, true));
        ta.recycle();
...
//        setFocusableInTouchMode(true);
    ...
    public float getStartDrawerWidthPercent() {
        ...
        if ((mFlags & FLAG_DRAWER_WIDTH_PERCENTAGES_RESOLVED) == 0) {
            if ((mFlags & FLAG_SUPPORTS_RTL) == 0 || isLayoutDirectionResolved(this)) {
    ...
    public boolean isDrawerSlidable(@EdgeGravity int gravity) {
        switch (gravity) {
            ...
            case Gravity.START:
            case Gravity.END:
                return isDrawerSlidable(getAbsoluteHorizontalGravity(this, gravity));
        }
        return false;
    }
...
//    }
    ...
    private boolean resolveDrawerWidthPercentagesIfDirectionResolved(boolean preventLayout) {
        final boolean directionResolved = isLayoutDirectionResolved(this);
    ...
    private void traverseAllChildren(int childCount, int layoutDirection) {
        if (childCount > 3) {
            throw new IllegalStateException("TomonDrawerLayout can host only three direct children.");
        }
...
//    }
    ...
    public void openDrawer(@EdgeGravity int gravity, boolean animate) {
        final int absoluteGravity = getAbsoluteGravity(this, gravity);
     ...
     * {@link OnDrawerScrollListener#onScrollPercentChange(TomonDrawerLayout, View, float)}
    ...
    public static class LayoutParams extends ViewGroup.LayoutParams {
        ...
        public LayoutParams(@NonNull Context c, @Nullable AttributeSet attrs) {
            super(c, attrs);
            TypedArray ta = c.obtainStyledAttributes(attrs, R.styleable.TomonDrawerLayout_Layout);
            gravity = ta.getInt(R.styleable.TomonDrawerLayout_Layout_android_layout_gravity,
                    Gravity.NO_GRAVITY);
            ta.recycle();
        }
...
//    }
    ...
    public interface OnDrawerScrollListener {
         ...
         */
        void onDrawerOpened(@NonNull TomonDrawerLayout parent, @NonNull View drawer);
         ...
         * @param drawer the drawer currently being dragged
         */
        void onDrawerClosed(@NonNull TomonDrawerLayout parent, @NonNull View drawer);
         ...
         */
        void onScrollPercentChange(@NonNull TomonDrawerLayout parent, @NonNull View drawer,
         ...
         */
        void onScrollStateChange(@NonNull TomonDrawerLayout parent, @NonNull View drawer,
    ...
    public static abstract class SimpleOnDrawerScrollListener implements OnDrawerScrollListener {
        @Override
        public void onDrawerOpened(@NonNull TomonDrawerLayout parent, @NonNull View drawer) {
        }

        @Override
        public void onDrawerClosed(@NonNull TomonDrawerLayout parent, @NonNull View drawer) {
        }

        @Override
        public void onScrollPercentChange(@NonNull TomonDrawerLayout parent,
                                          @NonNull View drawer, float percent) {
        }

        @Override
        public void onScrollStateChange(@NonNull TomonDrawerLayout parent,
    ...
    public CharSequence getDrawerTitle(@EdgeGravity int gravity) {
        final int absGravity = getAbsoluteGravity(this, gravity);
    ...
    public CharSequence getAccessibilityClassName() {
        return TomonDrawerLayout.class.getName();
    }
...
//            info.removeAction(AccessibilityNodeInfoCompat.AccessibilityActionCompat.ACTION_CLEAR_FOCUS);
        ...
        public boolean dispatchPopulateAccessibilityEvent(View host, AccessibilityEvent event) {
            ...
            // are concerned, state changes from TomonDrawerLayout invalidate the entire contents
